...
### 13. Outcome-Based Bias & The "Stuff+" Pivot
- **Pitfall:** Initial models relied exclusively on outcome-based metrics (SIERA, K-BB%, ERA). These features are "Lossy"—they capture what *happened* but fail to isolate a pitcher's raw physical *process*. A pitcher with elite velocity and movement could be penalized by the model due to a single high-variance game with poor defense or bad BABIP luck.
- **Lesson:** Machine learning models in sports must distinguish between **Process** (raw physical talent) and **Outcome** (noisy results). A robust model requires a "Talent Floor" that is independent of the defense or the stadium.
- **Fix:** Implemented a **Pitch-Level Modeling Layer**. By ingesting and training on millions of Statcast pitches (Velocity, Vertical Approach Angle, Extension, Spin), we created a proprietary **Stuff+** feature. This metric quantifies the "Probability of a Whiff" based solely on the physics of the pitch. By aggregating this back to the pitcher-season level, the master XGBoost engine now has an "Anchoring Feature" that remains stable even when traditional stats are noisy.
